...
class ServiceInterface:
    ...
    def __init__(self, _id: int, animal: Animal, data_de_inicio: datetime.datetime, valor: float):
        self.identifier = _id
        self.animal = animal
    ...
    def run(cls, animales: dict, registro: dict) -> None:
        ...
        service = cls(
            _id=len(registro[cls.__name__]) + 1,
            animal=animal,
            data_de_inicio=datetime.datetime.now(),
            valor=cls.get_price(),
        )
        registro[cls.__name__].append(service)
